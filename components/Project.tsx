  ...
  useEffect(() => {
    ...
    imagesRef.current.forEach((img:HTMLImageElement, i) => {
      if (!imageDimensions[i]){
        updatedDimensions[i] = { width:img.naturalWidth, height:img.naturalHeight, xyRatio: img.naturalWidth / img.naturalHeight };
      }
    ...
    if ( maxHeight ){
      ...
      setcontainerDimensions({
        ...
        xyRatio: ( windowSize.width  - 2.5 * windowSize.rem) / (maxHeight < 36 * windowSize.rem ? maxHeight : 36 * windowSize.rem)
      });
    }
  ...
  return (
    <Project>
      ...
      <ImageCarousel>
          ...
          <Images>
            {images.map((src, i) => {
              ...
              if ( imageDimensions[i] && containerDimensions ){
                let imageWidth = containerDimensions.xyRatio > imageDimensions[i].xyRatio ?
                ...
                if (imagePosition === 0 ){
                  ...
                  let totalLeft = containerDimensions.xyRatio > imageDimensions[activeImage].xyRatio ?
                    imageDimensions[activeImage].width * containerDimensions.height / imageDimensions[activeImage].height / 2 :
                    (containerDimensions.width) / 2;
                  for (let k = 1; k < Math.abs(imagePosition); k++){
                    totalLeft += containerDimensions.xyRatio > imageDimensions[activeImage - k].xyRatio ?
                      imageDimensions[activeImage - k].width * containerDimensions.height / imageDimensions[activeImage - k].height  * ((10 - k) / 10):
        ...
        <ImageList>
        <ImageSwitch
            onClick={() => activeImage - 1 >= 0 && setActiveImage(activeImage - 1)}>
            <Arrow />
          </ImageSwitch>
          <ImageListItem />
          <ImageSwitch
